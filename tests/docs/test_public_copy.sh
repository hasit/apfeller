...
assert_not_contains "$catalog_js" 'https://api.github.com/repos/hasit/apfeller-apps/releases?per_page=100' "catalog script should not fetch release stats"
assert_not_contains "$catalog_js" 'download_count' "catalog script should not read download counts from release assets"
assert_not_contains "$catalog_js" 'catalog-downloads' "catalog script should not render per-app download stats"
